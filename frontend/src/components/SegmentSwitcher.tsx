...
export const SegmentSwitcher: React.FC = () => {
  ...
  return (
    <div className="absolute bottom-4 left-2 sm:left-20 z-50">
      <button
        onClick={() => setIsOpen(!isOpen)}
        disabled={isLoading}
        className="bg-gray-800 hover:bg-gray-700 disabled:opacity-70 text-white px-2 sm:px-4 py-2 rounded-lg flex items-center gap-1 sm:gap-2 shadow-lg border border-gray-600 text-xs sm:text-sm"
      >
        {isLoading ? <Loader2 size={16} className="animate-spin sm:w-[18px] sm:h-[18px]" /> : <Layers size={16} className="sm:w-[18px] sm:h-[18px]" />}
        <span className="hidden sm:inline">Segments ({selectedCount}/{totalCount})</span>
      ...
      {isOpen && (
        <div className="absolute bottom-12 left-0 bg-gray-800 border border-gray-600 rounded-lg shadow-xl w-64 sm:w-72 overflow-hidden max-h-[70vh]">
