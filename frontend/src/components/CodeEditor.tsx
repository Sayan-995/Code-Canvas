...
export function CodeEditor() {
  ...
  return (
    <div className={`w-full h-screen h-[100dvh] bg-[#1e1e1e] text-white ${files.length === 0 ? 'overflow-y-auto' : 'overflow-hidden'}`}>
      ...
      {files.length === 0 ? (
        ...
        <>
          {!isFlowPlaying && (
            <div className="absolute top-2 sm:top-4 right-2 sm:right-4 z-50 flex flex-wrap gap-1.5 sm:gap-2 max-w-[calc(100%-1rem)] sm:max-w-none justify-end">
              {githubContext && (
                <>
                  <button
                    onClick={handlePullFromGithub}
                    disabled={isPulling || isPushing}
                    className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg flex items-center gap-1 sm:gap-2 transition-colors shadow-lg text-xs sm:text-sm"
                  >
                    {isPulling ? <Loader2 className="animate-spin" size={16} /> : <GitPullRequest size={16} className="sm:w-5 sm:h-5" />}
                    <span className="hidden sm:inline">Pull</span>
                  </button>
                  <button
                    onClick={handlePushToGithub}
                    disabled={isPushing || isPulling}
                    className="bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg flex items-center gap-1 sm:gap-2 transition-colors shadow-lg text-xs sm:text-sm"
                  >
                    {isPushing ? <Loader2 className="animate-spin" size={16} /> : <Github size={16} className="sm:w-5 sm:h-5" />}
              ...
              <button
                ...
                className="bg-green-600 hover:bg-green-700 text-white px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg flex items-center gap-1 sm:gap-2 transition-colors shadow-lg text-xs sm:text-sm"
              >
              ...
              <button
                ...
                className="bg-gray-600 hover:bg-gray-700 text-white px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg flex items-center gap-1 sm:gap-2 transition-colors shadow-lg text-xs sm:text-sm"
