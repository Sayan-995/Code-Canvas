...
export const UploadScreen: React.FC = () => {
  ...
  return (
    <div 
      className="relative flex flex-col items-center justify-start sm:justify-center min-h-screen min-h-[100dvh] bg-black text-white p-4 sm:p-6 md:p-8 overflow-y-auto overflow-x-hidden"
    ...
    >
      ...
      {/* Content overlay */}
      <div className={`relative z-10 flex flex-col items-center w-full py-8 sm:py-8 my-auto transition-opacity duration-700 ${transitionStage === 'fading' || transitionStage === 'complete' ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        <h1 className={`text-2xl sm:text-4xl md:text-5xl font-bold mb-4 sm:mb-8 md:mb-12 transition-all duration-500 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-600 text-center px-2 ${loading ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
          Code Canvas
        </h1>
        
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8 w-full max-w-4xl px-2 sm:px-4 pb-8 sm:pb-0 transition-all duration-500 ${loading ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'}`}>
        {/* Local Upload */}
        <div className="group bg-gray-900/50 backdrop-blur-sm p-4 sm:p-6 md:p-8 rounded-xl border border-gray-800 hover:border-blue-500/50 transition-all duration-300 flex flex-col items-center gap-3 sm:gap-4 hover:shadow-[0_0_30px_rgba(59,130,246,0.2)]">
          <div className="p-3 sm:p-4 rounded-full bg-blue-500/10 group-hover:bg-blue-500/20 transition-colors">
            <FolderUp size={36} className="text-blue-400 sm:w-12 sm:h-12" />
          </div>
          <h2 className="text-xl sm:text-2xl font-semibold">Upload Folder</h2>
          <p className="text-gray-400 text-center text-sm sm:text-base">Select a folder from your local machine to visualize.</p>
          <label className="cursor-pointer bg-blue-600 hover:bg-blue-500 text-white px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg font-medium transition-all hover:shadow-lg hover:shadow-blue-500/30 transform hover:-translate-y-0.5 text-sm sm:text-base">
            Choose Folder
            <input
              type="file"
              className="hidden"
        ...
        {/* GitHub Import */}
        <div className="group bg-gray-900/50 backdrop-blur-sm p-4 sm:p-6 md:p-8 rounded-xl border border-gray-800 hover:border-purple-500/50 transition-all duration-300 flex flex-col items-center gap-3 sm:gap-4 hover:shadow-[0_0_30px_rgba(168,85,247,0.2)]">
          <div className="p-3 sm:p-4 rounded-full bg-purple-500/10 group-hover:bg-purple-500/20 transition-colors">
            <Github size={36} className="text-purple-400 sm:w-12 sm:h-12" />
          </div>
          <h2 className="text-xl sm:text-2xl font-semibold">GitHub Repo</h2>
          <p className="text-gray-400 text-center text-sm sm:text-base">Import a repository to visualize and edit.</p>
          <div className="flex flex-col w-full gap-2 sm:gap-3">
            <input
              type="text"
              placeholder="https://github.com/owner/repo"
              className="w-full bg-black/50 border border-gray-700 rounded-lg px-3 sm:px-4 py-2.5 sm:py-3 focus:outline-none focus:border-purple-500 transition-colors text-sm sm:text-base"
              value={repoUrl}
            ...
            <input
              ...
              className="w-full bg-black/50 border border-gray-700 rounded-lg px-3 sm:px-4 py-2.5 sm:py-3 focus:outline-none focus:border-purple-500 text-xs sm:text-sm transition-colors"
              value={githubToken}
            ...
            <button
              ...
              className="w-full bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white px-4 py-2.5 sm:py-3 rounded-lg font-medium transition-all hover:shadow-lg hover:shadow-purple-500/30 transform hover:-translate-y-0.5 text-sm sm:text-base"
            >
              Import Repository
            </button>
          </div>
        ...
        {loading && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-3 sm:p-4">
            <div className="w-full max-w-2xl bg-[#0d1117] rounded-lg border border-gray-800 shadow-2xl overflow-hidden font-mono text-xs sm:text-sm mx-2">
              {/* Terminal Header */}
              <div className="bg-[#161b22] px-3 sm:px-4 py-2 flex items-center gap-2 border-b border-gray-800">
                ...
                <div className="ml-2 sm:ml-4 text-gray-400 text-[10px] sm:text-xs truncate">~/kiro/system-analysis</div>
              </div>
              
              {/* Terminal Body */}
              <div 
                ref={terminalRef}
                className="p-3 sm:p-6 h-60 sm:h-80 overflow-y-auto flex flex-col gap-1.5 sm:gap-2 font-mono scrollbar-hide"
        ...
        {error && (
          <div className="mt-4 sm:mt-8 text-red-400 bg-red-900/20 px-4 sm:px-6 py-3 sm:py-4 rounded-lg border border-red-500/50 backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4 text-sm sm:text-base mx-4">
            {error}
          </div>
        )}
      </div>
